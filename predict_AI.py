...
for tif_file in tif_files:
    ...
    output_path1 = os.path.join("/media/xsar/F/guo-x/rock_seg/segformer-pytorch/pre_dection_result/b0-2/",f"{os.path.splitext(tif_file)[0]}_1.tif")
    output_path2 = os.path.join("/media/xsar/F/guo-x/rock_seg/segformer-pytorch/pre_dection_result/b0-2/",f"{os.path.splitext(tif_file)[0]}_2.tif")
    output_path3 = os.path.join("/media/xsar/F/guo-x/rock_seg/segformer-pytorch/pre_dection_result/b0-2/",f"{os.path.splitext(tif_file)[0]}_3.tif")
...
# 打开影像文件
    ...
    # 创建输出影像
    output_path4 = os.path.join("/media/xsar/F/guo-x/rock_seg/segformer-pytorch/pre_dection_result/b0-2/",f"{os.path.splitext(tif_file)[0]}_4.tif")
    driver = gdal.GetDriverByName('GTiff')
    output_ds = driver.Create(output_path4, image_width, image_height, 1, gdal.GDT_Byte)
    output_ds.SetGeoTransform(geo_transform)  # 设置地理变换参数
    output_ds.SetProjection(projection)  # 设置投影信息
    ...
    band3_ds = None
    output_ds = None
    print("影像相加完成，结果保存在", output_path4)
    ...
    # 打开tif图像
    input_image_path = output_path4
    output_path5 = os.path.join("/media/xsar/F/guo-x/rock_seg/segformer-pytorch/pre_dection_result/b0-2/",f"{os.path.splitext(tif_file)[0]}_5.tif")
    
    
    input_ds = gdal.Open(input_image_path, gdal.GA_ReadOnly)# 打开输入图像
    # 读取输入图像数据为NumPy数组
    ...
    # 将NumPy数组写入输出图像
    output_ds.GetRasterBand(1).WriteArray(np.array(input_array))
